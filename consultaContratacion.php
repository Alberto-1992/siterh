...
$conexionDocumentacion = new ConexionDocumentacion();
    $sqlQueryComentarios  = $conexionDocumentacion->prepare("SELECT datospersonales.id_datopersonal FROM datospersonales where datosActualizados = 3 ");
    ...
    if(isset($_POST['evento']))
{
	...
	$query=$conexionDocumentacion->prepare("SELECT datospersonales.id_datopersonal, datospersonales.curp, datospersonales.nombre, datospersonales.appaterno, datospersonales.apmaterno, datospersonales.correoelectronico, datospersonales.datosActualizados FROM datospersonales WHERE
        datospersonales.nombre LIKE '%$q%' and datosActualizados = 3 OR
        datospersonales.correoelectronico LIKE '%$q%' and datosActualizados = 3 OR
        datospersonales.curp LIKE '%$q%' and datosActualizados = 3 OR
		datospersonales.appaterno LIKE '%$q%' and datosActualizados = 3 OR
		datospersonales.apmaterno LIKE '%$q%' and datosActualizados = 3 group by datospersonales.id_datopersonal");
}
        ?>
...
$sql = $conexionDocumentacion->prepare("SELECT id_datopersonal from datospersonales where datosActualizados = 3 order by id_datopersonal desc limit 1");
        $sql->execute();
            $row = $sql->fetch();
...
$(function() {
    var id = $("#cargaPrimerRegsitro").val();
        ...
        $.ajax({
            type: "POST",
            url: "consultaBusquedaContratacion.php",
            data: ob,
            beforeSend: function() {
    ...
    $("#tabla_resultadobus").on("scroll", function() {
        ...
        if ((((scrollHeight - 250) >= scrollPos) / scrollHeight == 0) || (((scrollHeight - 300) >=
            ...
            if ($(".item-comentario").length < $("#total_registro").val()) {
                ...
                $("#tabla_resultadobus").off("scroll");
                $.ajax({
                    url: 'obteniendoMasDatosContratacion.php',
                    data: datos,
                    type: "POST",
                    beforeSend: function() {
                        $('.ajax-loader').show();
